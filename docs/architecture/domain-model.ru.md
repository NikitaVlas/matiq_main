...
### Приоритет рекомендаций после просмотра урока

После завершения урока MATIQ показывает рекомендации в следующем порядке:

1. `Следующее в вашем Roadmap` — основная рекомендация, если у спортсмена есть актуальный
   незавершённый элемент Roadmap.
2. `Продолжить эту техническую связку` — дополнительная рекомендация из основного или условного
   lesson path просмотренного урока.
3. Подбор по метаданным — резервный вариант, когда нет ни актуального элемента Roadmap, ни
   подходящего lesson path.

Если активного Roadmap нет, подходящий lesson path становится основной рекомендацией. Интерфейс
разделяет рекомендации Roadmap и технической связки и объясняет источник каждой рекомендации.
Воспроизведение не запускается автоматически: пользователь явно выбирает следующую рекомендацию.
...
- `DM-015`: После просмотра актуальный элемент Roadmap имеет приоритет над рекомендацией из lesson path.
- `DM-016`: Рекомендация никогда не запускает воспроизведение без явного действия пользователя.
...
- Связанный код: Methodology, content, Roadmap; реализован первый срез Course/Lesson
